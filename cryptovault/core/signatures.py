...
def create_detached_signature(file_path: str | Path, private_key: ed25519.Ed25519PrivateKey, destination: str | Path) -> None:
    digest = bytes.fromhex(hash_file(file_path, "SHA-512"))
    document = {
...
def verify_detached_signature(file_path: str | Path, signature_path: str | Path, public_key: ed25519.Ed25519PublicKey) -> bool:
    ...
    actual = bytes.fromhex(hash_file(file_path, document["digest_algorithm"]))
