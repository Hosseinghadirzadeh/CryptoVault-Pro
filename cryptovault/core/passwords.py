...
def derive_key(password: str, salt: bytes, method: str, params: dict | None = None) -> tuple[bytes, dict]:
    ...
    if method == "Argon2id":
        ...
        effective = {"n": 2**15, "r": 8, "p": 1, "maxmem": 128 * 1024 * 1024, **params}
        ...
        if not 32 * 1024 * 1024 <= int(effective["maxmem"]) <= 512 * 1024 * 1024:
            raise ValueError("scrypt maxmem is outside the safe range")
